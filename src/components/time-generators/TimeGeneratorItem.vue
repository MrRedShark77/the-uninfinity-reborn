...
import { timeGeneratorTextures } from '@/data/textures';
import { TIME_GENERATOR } from '@/data/generators/time-generators';

const { n } = defineProps<{'n' : number}>()

const GEN = TIME_GENERATOR(n);
...
<template>
  <div class="g--generator" v-if="GEN.unlocked">
    <PrimaryButton class="g--generator-button g--eternity-button top" :enabled="Decimal.gte(GEN.resource,cost)" @click="GEN.purchase(false)">
      <img class="image" :src="timeGeneratorTextures[n]" draggable="false">
      <div class="g--gen-top">
    ...
    <PrimaryButton class="g--generator-button g--eternity-button bottom" @click="GEN.purchase(true)">Buy Max</PrimaryButton>
  </div>
</template>
