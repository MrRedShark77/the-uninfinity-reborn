...
export const EXPANDER = {
  ...
  get requirement(): [number, DecimalSource] {
    ...
    const m = Decimal.sub(this.amount, NC9 ? 4 : 6).max(0).mul(isICBeaten(5) ? 1.4 : 1.5).add(2).ceil()
  ...
  get bulk(): DecimalSource {
    ...
    const x = Decimal.sub(amount, 2).div(isICBeaten(5) ? 1.4 : 1.5).add(NC9 ? 4 : 6).floor().add(1)
...
export const REFINER = {
  ...
  calc_boost(amount: DecimalSource): DecimalSource {
    ...
    x = x.div(L).root(isICBeaten(2) ? 2.5 : 3).mul(10).sub(10).pow10()
